...
export default function QuizPageforDataScience() {
  ...
  const handleSubmitButtonClick = async (e, index) => {
    ...
    dataScienceQuizData?.forEach((quiz, indexMap) => {
      if (
        quiz?.password === checkPassword &&
        indexMap === index &&
        !enteredPassword.includes(checkPassword)
      ) {
        isCorrect = true;
        setenteredPassword((prev) => [...prev, checkPassword]);
        const submittedFormsIndex =
          JSON.parse(localStorage.getItem("submittedFormsIndex")) || [];
        submittedFormsIndex.push(index);
        localStorage.setItem(
          "submittedFormsIndex",
          JSON.stringify(submittedFormsIndex)
        );
  ...
  return (
    <div>
      ...
      <div className=" p-10 mt-[5%] max-md:mt-[50px] font-Montserrat grid place-content-center w-full ">
        <h1 className=" text-center text-3xl font-Andika ">
            ...
            >
              ...
              <div>
                <a
                  href={link.formlink}
                  target="_blank"
                  className=" cursor-pointer text-blue-600 underline underline-offset-2 "
                >
                  {link.name}
                </a>
              </div>
              ...
              {!answeredFormsIndex?.includes(index) ? (
                ...
                <button className=" font-Andika bg-gradient-to-r p-4 from-amber-400 via-green-500 to-indigo-400 text-transparent bg-clip-text font-bold text-xl ">
                  Already Done
                </button>
              )}
            </div>
        ...
        <div className=" mt-24 w-4/6 mx-auto ">
          {answeredFormsIndex?.length > 0 && (
            <div className=" leading-10  ">
              <h1 className=" bg-gradient-to-br text-transparent bg-clip-text text-4xl font-bold font-Andika from-[#01D796] via-orange-400 to-pink-300 ">
                Clues:
              </h1>
              {dataScienceClues
                .slice(0, Math.floor(answeredFormsIndex?.length / 2))
                .map((clue, index) => {
                  return (
                    <h2 key={index} className=" text-center ">
                      {clue.includes("https") ? (
                        <a
                          className=" cursor-pointer text-lg font-Montserrat font-semibold "
                          href={clue}
                          target="_blank"
                        >
                          {clue}
                        ...
                        <p className="text-lg font-Montserrat font-semibold cursor-default ">
                          {clue}
                        </p>
                      )}
                    </h2>
