        ...
        <div className="flex justify-center w-4/6 mx-auto  ">
          <div className=" absolute m-10  ">
            <img
              src={bgGirl}
              alt="bg-image"
              className="flex justify-center opacity-15 "
            />
          </div>
          <div className=" absolute max-md:bg-background-color  ">
            <div className=" w-5/6 mx-auto">
              <h1 className="text-4xl font-bold">{quizData[index].question}</h1>
              <div className=" flex items-center  justify-center space-x-10 mt-10 ">
                {quizData[index]?.supportedMethods?.length > 0 && (
                  ...
                  >
                    {quizData[index].supportedMethods.map(
                      (httpmethod, index) => {
                        return <option key={index}>{httpmethod}</option>;
                      }
                    )}
                  </select>
                )}
                ...
                />
                <button onClick={handleAnswerSubmissison} className=" ">
            ...
            {hintsIndex < quizData[index].hints?.length - 1 && (
              ...
              >
                <h1 className=" font-semibold text-3xl  p-4 text-center cursor-pointer">
            ...
            {showHint && (
              <div className={` ${showHint ? "" : "hidden"}  `}>
                {quizData[index].hints
                  ?.slice(0, hintsIndex + 1)
                  .map((hint, index) => {
                    return (
                      <div key={index}>
                        <h1 className=" text-2xl font-Montserrat text-center p-5 ">
                          {hint}
                        </h1>
                      </div>
                    );
